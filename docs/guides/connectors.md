...
3. **每次请求前重新校验** —— 只在登记时查一次挡不住 DNS 重绑定
...
**残留风险**：第 3 步的校验与实际连接之间有一个 TOCTOU 窗口（两次独立的 DNS 查询）。
彻底消除要自己接管 socket 层的 lookup 并把连接钉死到已校验的 IP。当前取舍是：
内部平台、调用者必须是已登录员工、每次调用留审计。**如果哪天要对外开放，这里必须先补。**
