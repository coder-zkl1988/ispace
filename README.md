...
**MCP**（23 个工具）——`/deploy/mcp`，复用调用者身份，只能操作本人空间，
每次调用进审计日志：
...
不加限制就是一台内网扫描器，所以有四道防线：只允许 http/https；拒绝一切解析到
私有段/回环/链路本地（含云厂商元数据地址 169.254.169.254）的目标；**判定发生在
`net.connect` 的 lookup 钩子里**——钩子返回哪个地址内核就连哪个地址，不存在
"先检查后使用"的缝，DNS 重绑定无从下手；不跟随重定向（否则公网主机 302 到
